...
class KZ_2(Base_model):
    ...
    def __init__(self, init_x, init_y, position, canv, root, initial_list_wires, initial_control_actions, initial_initial_conditions, initial_secondary_parameters, Comdobox_index):
        ...
        Base_model.__init__(self, init_x, init_y, canv, root, "Image/KZ_2/", coord, position, list_nodes, list_graph, initial_secondary_parameters, "KZ", list_text_control_actions, list_text_initial_conditions, initial_control_actions, initial_initial_conditions, initial_list_wires, coords_text, Comdobox_index)
    ...
    def get_main_determinant(self, input_variable, t):
        main_determinant = np.array([[2*(self.L + self.Loff)]], dtype = self.data_type)
        return main_determinant

    def get_own_matrix(self, input_variable, t):
        self.help_ss(t)

        own_matrix = np.array([-input_variable[0]*2*(self.R+self.Roff)], dtype = self.data_type)
        return own_matrix
